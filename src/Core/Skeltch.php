...
class Skeltch
{
    ...
    public static function run(string $filename)
    {
        ...
        if (!SkeltchGo::getCache() || !is_file($tmpfile) || filemtime($tmpfile) < filemtime($filename)) self::compile($filename, $tmpfile);
    ...
    private static function compileForEachs(string $code)
    {
        return preg_replace_callback('~(?<!@){\s*foreach\s*\((.+?)\s+as\s+(.+?)(\s+=>\s+(.*?))?\)\s*}~is', function ($matches) {
            if (isset($matches[4])) {
                return sprintf('<?php \Glowie\SkeltchGo\Core\Skeltch::resetLoop(); foreach(%s as %s => %s): $loop = \Glowie\SkeltchGo\Core\Skeltch::getLoop(%s); ?>', $matches[1], $matches[2], $matches[4], $matches[1]);
            } else {
                return sprintf('<?php \Glowie\SkeltchGo\Core\Skeltch::resetLoop(); foreach(%s as %s): $loop = \Glowie\SkeltchGo\Core\Skeltch::getLoop(%s); ?>', $matches[1], $matches[2], $matches[1]);
            }
